...
def conv_block(
    ...
    if conv_type == "ds":
        out = tf.keras.layers.SeparableConv2D(
            filters=filters, kernel_size=kernel_size, padding=padding, strides=strides
        )(inputs)
    elif conv_type == "conv":
    ...
    if upsampling:
        out = tf.keras.layers.UpSampling2D(
            size=(up_sample_size, up_sample_size), data_format="channels_last"
        )(out)
        skip_layer = tf.keras.layers.Conv2D(
            filters=out.shape[3], kernel_size=(1, 1), padding="same", strides=(1, 1)
        )(skip_layer)
...
def residual_bottleneck(
    inputs, filters=64, kernel_size=(3, 3), strides=(1, 1), temp=3, relu=False
):
    """
    ...
    out = conv_block(inputs, conv_type="conv", filters=tchannel, kernel_size=(1, 1))
    out = tf.keras.layers.DepthwiseConv2D(
        kernel_size=kernel_size, depth_multiplier=1, strides=strides, padding="same"
    )(out)
    out = tf.keras.layers.BatchNormalization()(out)
    out = tf.keras.activations.relu(out)
    out = conv_block(
...
def bottleneck_block(
    inputs, filters=64, kernel_size=(3, 3), strides=(1, 1), temp=6, loop=3
):
    """
    ...
    for _ in range(1, loop):
        out = residual_bottleneck(
            inputs=out, filters=filters, kernel_size=kernel_size, temp=temp, relu=True
        )
    return out
...
class CustomDotLayer(tf.keras.layers.Layer):
    def call(self, inputs):
        x, y = inputs
        return tf.keras.backend.dot(x, y)


class CustomTransposeLayer(tf.keras.layers.Layer):
    def call(self, inputs):
        return tf.keras.backend.transpose(inputs)
...
def tem_block(inputs):
    ...
    conv_1 = tf.keras.layers.Conv2D(
        filters=256, kernel_size=(3, 3), padding="same", name="conv_1"
    )(inputs)
    ...
    transpose_1 = CustomTransposeLayer()(reshape_1)
    mat_mul_1 = CustomDotLayer()([transpose_1, reshape_1])

    conv_4 = tf.keras.layers.Conv2D(
        filters=256, kernel_size=(1, 1), padding="same", name="conv_4"
    )(add_1)
    ...
    mat_mul_2 = CustomDotLayer()([reshape_2, mat_mul_1])
...
def feature_extractor(input_layer):
    ...
    """
    fe_layer1 = bottleneck_block(
        input_layer, filters=48, kernel_size=(3, 3), strides=(2, 2), temp=6, loop=3
    )
    fe_layer2 = bottleneck_block(
        fe_layer1, filters=64, kernel_size=(3, 3), strides=(2, 2), temp=6, loop=3
    )
    fe_layer3 = bottleneck_block(
        fe_layer2, filters=96, kernel_size=(3, 3), strides=(1, 1), temp=6, loop=3
    )
    fe_layer4 = pyramid_pooling_block(input_tensor=fe_layer3, bin_sizes=[2, 4, 6, 8])
...
def fusion_module(learning_layer, fe_layer):
    ...
    fusion_layer2 = tf.keras.layers.DepthwiseConv2D(
        kernel_size=(3, 3), strides=(1, 1), depth_multiplier=1, padding="same"
    )(fusion_layer2)
    fusion_layer2 = tf.keras.layers.BatchNormalization()(fusion_layer2)
    fusion_layer2 = tf.keras.activations.relu(fusion_layer2)
    fusion_layer2 = tf.keras.layers.Conv2D(
        filters=96, kernel_size=(1, 1), strides=(1, 1), padding="same", activation=None
    )(fusion_layer2)
...
def get_encoder(image_height, image_width):
    ...
    # learning module
    learning_layer1, learning_layer2, learning_layer3 = learning_module(
...
def get_decoder(encoder, learning_layer1, learning_layer2, num_classes):
    ...
    classifier2 = tf.keras.layers.UpSampling2D(
        size=(2, 2), data_format="channels_last"
    )(classifier2)

    classifier3 = tf.keras.layers.Conv2D(
        filters=num_classes, kernel_size=(1, 1), padding="same", strides=(1, 1)
    )(classifier2)
    ...
    model = tf.keras.models.Model(
        inputs=[inputs], outputs=[classifier3], name="pcb-decoder"
    )
    return model
