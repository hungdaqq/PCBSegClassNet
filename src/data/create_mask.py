    ...
    transform = A.Compose(
        [
            A.augmentations.transforms.CLAHE(
                clip_limit=4.0, tile_grid_size=(8, 8), always_apply=False, p=1.0
            )
        ]
    )
                ...
                if os.path.exists(os.path.join(source_image_dir, image_name[0])):
                    ...
                    for anote, cat in zip(vertices_list, designator_list):
                        if cat in color_values:
                            ...
                            continue
                        try:
                            pts = np.array(ast.literal_eval(anote))[0].reshape(
                                (-1, 1, 2)
                            )
                        except:
                            continue
                        ...
                        cv2.imwrite(
                            os.path.join(crops_dest_dir, cat, f"image_{cnt}.png"),
