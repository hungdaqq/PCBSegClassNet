...
def create_patches(
    images_list, masks_list, patch_size, dest_images_dir, dest_masks_dir
):
    """
            ...
            for j in range(0, img.shape[0], patch_size):
                for k in range(0, img.shape[1], patch_size):
                    ...
                    if temp.shape[1] != patch_size:
                        ...
                        temp = img[j : j + patch_size, k : k + patch_size, :]
                    cv2.imwrite(
                        os.path.join(dest_images_dir, f"image_{count}.png"),
                        cv2.cvtColor(
                            img[j : j + patch_size, k : k + patch_size, :],
                            cv2.COLOR_BGR2RGB,
                        ),
                    )
                    cv2.imwrite(
                        os.path.join(dest_masks_dir, f"image_{count}.png"),
                        cv2.cvtColor(
                            msk[j : j + patch_size, k : k + patch_size, :],
                            cv2.COLOR_BGR2RGB,
                        ),
                    )
                    count += 1
...
def split_data(
    ...
    masks_list = sorted(
        [
            os.path.join(source_masks_dir, image)
    ...
    train_images, test_images, train_masks, test_masks = train_test_split(
        images_list, masks_list, test_size=split_percentage, random_state=42
    )

    train_crops, test_crops, train_labels, test_labels = train_test_split(
        crops_list, crops_labels_list, test_size=split_percentage, random_state=42
    )
...
def main(images_dir, masks_dir, crops_dir, patch_size=768):
    ...
    """
    dest_images_dir = os.path.join(
        os.path.abspath(os.path.join(images_dir, os.pardir)), "patches", "images"
    )
    dest_masks_dir = os.path.join(
        os.path.abspath(os.path.join(masks_dir, os.pardir)), "patches", "masks"
    )
    ...
    masks_list = sorted(
        [os.path.join(masks_dir, image) for image in os.listdir(masks_dir)]
    )
